...
\subsection{Proof-of-concept Experiments with Smaller Codes}
...
\subsubsection{Simulating Toy Version of Grover's Algorithm}
The circuit design for Grover's algorithm without any form for error correction is well-known, and is illustrated in \cref{sec:grovers_explanation}.
\todo[inline]{How was it implemented? Add a diagram and maybe some pseudocode.}

\section{Utilizing Stabilizer Formalism as a Computational Tool}\label{chap:stabilizers_in_computation}
...
\subsection{Simulating Stable Quantum Memory with PanQEC}
A natural place to begin is with quantum memory. As a baseline and to verify the correctness of our \texttt{PanQEC} implementation, we do a quantum memory threshold simulation %of \textcite{sommerstudentNSM}%using the smallest available codes, namely the surface code displayed in \cref{fig:smallest_surface_code}, and the $[[16, 4, 4]]$ BB code, the smallest BB code identified by \textcite{liang2026selfdualbivariatebicyclecodes}. Second, we extend this comparison to a more realistic scale 
where we compare the gross code to the surface code with identical logical qubit counts, in order to say something meaningful about the architectural trade-offs relevant to Jaques' assumptions. The error model applied is known as \emph{depolarizing} error\todo{more explanation?!!}, and is the same across all experiments. In addition, all experiments use the BP-LSD decoder\todo{more explanation needed?}. We compare logical error rate as a function of physical error rate under Monte Carlo sampling. 
...
\section{The Trouble with the T-gate}\label{chap:t_gate}
