...
That five qubits is the minimum required for this purpose is not immediately obvious, but follows from a counting argument. Correcting an arbitrary single-qubit error on $n$ qubits requires distinguishing between $3n+1$ error syndromes (the three error types $X$, $Y$ and $Z$ on each of the $n$ qubits, plus the no-error case). With $n - k$ stabilizer generators, the syndrome space has $2^{n-k}$ elements. For $k = 1$ logical qubit, we therefore require $2^{n-1} \geq 3n + 1$, which is first satisfied at $n = 5$.
...
The $T$-gate, as discussed in \cref{sec:t_gate}, is not transversal in the Steane code, as is guaranteed by the Eastin-Knill theorem, and its implementation requires the magic state distillation techniques described there.