...
We can show that by applying $\mathcal{G}$ on the $\ket{A_0}$ and $\ket{A_1}$, we obtain the following linear combinations \cite{Qiskit2023}:
\begin{align*}
...
The continued usage of the surface code at the current correction rates is one of the underlying assumptions in Jaques' predictions. Thus, it is interesting for the sake of our analysis to see how the prognosis for Grover's algorithm changes with a different choice of error correcting code. A few promising examples can be found in the \emph{Low Density Parity Check} (LDPC) family of codes, of which the surface code is a special case\todo{Explain and introduce LDPC codes properly.}. 

In particular, \emph{Bivariate Bicycle code}, also known as BB code, is a strong potential candidate \cite{Bravyi2024}. Though similar to surface codes at a high level of abstraction, BB codes differ in that their stabilizers are not geometrically local, and has therefore been called ``surface code with extra long-distance checks'' \cite{sommerstudentNSM}. These checks allow for a number of advantages over the surface code, such as a higher threshold for error correction and lower qubit overhead \cite{Bravyi2024}. 
...
Assuming we have created some scheme for fault-tolerant storage of quantum data, we still face a number of significant challenges before we can perform any fault-tolerant computations. In particular, the \emph{Eastin-Knill theorem} \cite{Eastin_2009,eastinknillwiki} is commonly paraphrased as follows:
\begin{center}
    \begin{tikzpicture}
    \node [mybox] (box){%
        \begin{minipage}{0.85\textwidth}
    ...
    \node[fancytitle, right=10pt] at (box.north west) {The Eastin-Knill theorem (paraphrased\footnote{Formal statement: For any nontrivial local-error-detecting-code, the set of logical unitary product operators is not universal.})};
    \end{tikzpicture}%
\end{center}
...
The Eastin-Knill theorem tells us that we cannot have a universal gate set in which every gate is \emph{transversal}, where transversality refers to the property that a gate that can transform a logical qubit by applying a gate to each of the $n$ physical qubits encoding it in a specific error correcting code. The physical qubits encoding a single logical qubit are collectively called a \emph{code block} or \emph{error-correcting patch}, which we will use interchangeably. A gate acting on $k$ logical qubits therefore acts across $k$ such blocks. A transversal gate can be expressed like this in a particular error correcting code:
\[G_L = \bigotimes^n_{i=1} G_i^{(1, 2, ..., k)}.
\]
where $G_i^{(1, 2, ..., k)}$ acts only on the $i$-th qubit in each of the $k$ blocks, with no coupling between positions $i\neq j$ within any single block. The transversal property is illustrated for a single- and multi-qubit gate in \cref{fig:transversalitydiagram}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/VIER PFOTEN_2019-03-15_001-2901x1934-2802x1934-1920x1325.jpg}
    \caption{Illustration of transversality in a single- and multi-qubit gate. \textcolor{red}{Update image and add source.}}\label{fig:transversalitydiagram}
\end{figure}
...
It is important to emphasize that transversality is not an inherent property of a gate, but rather a relationship between a gate and a particular error correcting code. Thus, we would like to choose the error correcting architecture in such a way that as many commonly used gates as possible are transversal in it. 


LIST OF THINGS TO INCLUDE

\sectionoutline{We need the link between transtransversality (a logical gate is applied by applying the same gate to its constituent physical qubits individually) and being able to manipulate the stabilizers only in order to alter the logical state the patch contains. To introduce this, we need to understand Clifford gates and how they work in the stabilizer formalism.}
...
\todo[inline]{dismantle this section entirely! move clifford+T discussion to previous section and move other software/challenges discussion to experiments and analysis part. Then get some of the theory in experiment and analysis and bring it back up here.}
...
\section{Logical Operators in Error Correcting Architecture}
...
\subsection{Implementing Clifford Gates in LDPC Codes}
As highlighted earlier in \cref{chap:grovers_explanation}\supervisors{Might add a figure name here}, the gates we need to perform the full circuit for Grover's algorithm are $H$, $Z_f$ and $Z_{\text{OR}}$. Of these, $H$ is largely considered computationally easy, taking our standard $X$ and $Z$ corrections and swapping them, since $H$ maps $\ket{0}$ to $\ket{+}$, $\ket{1}$ to $\ket{-}$, and vice versa, leaving us in the realm of the computationally easy Clifford gates. 

We cannot implement Clifford gates transversally in the 5-qubit code.
...
\chapter{Jaques' Argument}
\todo[inline]{Not sure whether to include as a separate section or work parts of it into the intro/background/discussion wherever necessary.}
