%\chapter{Background}
\chapter{Grover's Algorithm for Solving Unstructured Search}\label{chap:grovers_explanation}
\textit{\Cref{chap:grovers_explanation} is adapted from parts of my previous work \cite{ownwork}, with revisions and additions for clarity.}
...
\section{The Algorithm and Circuit}\label{sec:grovers}
\begin{center}
    \begin{tikzpicture}
    \node [mybox] (box){%
        \begin{minipage}{0.85\textwidth}
...
\subsection{Bivariate Bicycle Code: An Alternative to Toric Surface Code}
...
\section{Universal Gate Sets}\label{sec:universalgates}
...
which we return to in \cref{chap:t_gate}.
...
\chapter{Jaques' Argument}\label{chap:jaques}
\supervisorsinline{Do I include a section like this? If so, where? Parts of it should probably go in the introduction, but I feel like I should go through some parts of it in technical detail, which can really only be done after the background section you have now been through.}
